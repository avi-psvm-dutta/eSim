...
from subcircuit.Subcircuit import Avipanel
...
class DockArea(QtGui.QMainWindow):
    ...
    def avimessage(self):
        """
        This function creates avi's options
        """

        global count
        self.aviWidget=QtGui.QWidget()
        self.aviLayout=QtGui.QVBoxLayout()
        self.aviLayout.addWidget(Avipanel(self))
        
        self.aviWidget.setLayout(self.aviLayout)
        dock['Avi-'+str(count)] = QtGui.QDockWidget('Avi-'+str(count))
        dock['Avi-'+str(count)].setWidget(self.aviWidget)
        self.addDockWidget(QtCore.Qt.TopDockWidgetArea, dock['Avi-'+str(count)])  
        self.tabifyDockWidget(dock['Welcome'],dock['Avi-'+str(count)])
        
        #CSS
        dock['Avi-'+str(count)].setStyleSheet(" \
        .QWidget { border-radius: 15px; border: 1px solid gray; padding: 5px; width: 200px; height: 150px;  } \
        ")
        
        dock['Avi-'+str(count)].setVisible(True)
        dock['Avi-'+str(count)].setFocus()
        dock['Avi-'+str(count)].raise_()
